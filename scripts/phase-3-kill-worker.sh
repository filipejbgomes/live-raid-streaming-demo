...
pod=$(kubectl -n flink get pods -l app=bossraid-score-engine,component=taskmanager -o jsonpath='{.items[0].metadata.name}')
[[ -n "$pod" ]] || { echo 'No TaskManager found' >&2; exit 1; }
kubectl -n flink delete pod "$pod" --wait=true
sleep 10
wait_job
kubectl -n flink wait pod -l app=bossraid-score-engine,component=taskmanager --for=condition=Ready --timeout=180s
kubectl -n flink get pods -l app=bossraid-score-engine

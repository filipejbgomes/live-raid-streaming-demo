...
class Proof:
    ...
    def report(self):
        ...
        expected=q("SELECT count(*) FROM events WHERE topic='raid-attacks'")
        output=q("SELECT count(*) FROM events WHERE topic='raid-score-updates'")
        integrity=q("SELECT count(*) FROM events WHERE topic='raid-integrity'")
        duplicates=q('SELECT coalesce(sum(value),0) FROM counters')
        missing=q("SELECT count(*) FROM events a LEFT JOIN events b ON a.id=b.id AND b.topic='raid-score-updates' WHERE a.topic='raid-attacks' AND b.id IS NULL")
        extra=q("SELECT count(*) FROM events b LEFT JOIN events a ON a.id=b.id AND a.topic='raid-attacks' WHERE b.topic='raid-score-updates' AND a.id IS NULL")
        gaps=q("SELECT coalesce(sum(mx-n),0) FROM (SELECT max(seq) mx,count(DISTINCT seq) n FROM events WHERE topic='raid-attacks' GROUP BY player)")
        mismatches=q('''SELECT count(*) FROM events a JOIN events b ON a.id=b.id AND b.topic='raid-score-updates'
            WHERE a.topic='raid-attacks' AND (a.hash!=b.hash OR b.version NOT IN ('v1','v2') OR b.applied!=a.damage*CASE WHEN b.version='v2' AND a.seq%10=0 THEN 2 ELSE 1 END)''')
        integrity_errors=q('''SELECT count(*) FROM events a LEFT JOIN events b ON a.id=b.id AND b.topic='raid-integrity'
            WHERE a.topic='raid-score-updates' AND (b.id IS NULL OR a.payload!=b.payload)''')
        invalid=q("SELECT count(*) FROM events WHERE topic='raid-score-updates' AND valid=0")
        damage=q("SELECT coalesce(sum(applied),0) FROM events WHERE topic='raid-score-updates'")
        rows=self.db.execute("SELECT payload FROM events WHERE topic='raid-score-updates' ORDER BY json_extract(payload, '$.totalAttacks') DESC LIMIT 1").fetchall()
        ...
        state_errors=q("""SELECT count(*) FROM (
            ...
            FROM events WHERE topic='raid-score-updates')
        ...
        leaders=self.db.execute("SELECT player,sum(applied) d FROM events WHERE topic='raid-score-updates' GROUP BY player ORDER BY d DESC LIMIT 10").fetchall()

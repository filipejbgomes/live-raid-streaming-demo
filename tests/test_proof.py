...
class ProofTests(unittest.TestCase):
    ...
    def add(self,e,topics=None):
        for t in topics or ['raid-attacks','raid-score-updates','raid-integrity']:self.p.ingest(t,0,e['sequence']-1,e)
    ...
    def test_missing_output_fails(self):self.add(self.event(),['raid-attacks']);self.assertEqual(self.p.report()['missingAttacks'],1)
    def test_duplicate_distinct_offset_fails(self):
        e=self.event();self.add(e);self.p.ingest('raid-score-updates',0,2,e);self.assertEqual(self.p.report()['duplicateAttacks'],1)
    ...
    def test_corrupt_payload_fails(self):
        e=self.event();self.add(e,['raid-attacks']);e['damage']=20;self.add(e,['raid-score-updates','raid-integrity']);self.assertEqual(self.p.report()['checksum'],'MISMATCH')
    ...
    def test_integrity_payload_disagreement_fails(self):
        e=self.event();self.add(e,['raid-attacks','raid-score-updates']);e['bossHp']=7;self.add(e,['raid-integrity']);self.assertFalse(self.p.report()['proofPass'])
    ...
    def test_unexpected_output_fails(self):self.add(self.event(),['raid-score-updates','raid-integrity']);self.assertFalse(self.p.report()['proofPass'])

...
Use `./scripts/reset-demo.sh` for a fresh local presentation. It removes and recreates the four demo namespaces, operators, Kafka, storage and workloads while keeping the K3s node and its image cache. Kafka history, checkpoints, verifier evidence and all demo pod restart counts start at zero. Use `./scripts/reset-demo.sh --cluster` only when a new K3s node is also required; that mode repulls node-level images. It only operates on the named local k3d cluster. `./scripts/cleanup.sh --cluster` deletes that cluster without rebuilding it. Never clear Kafka alone while keeping Flink state.

Default local cluster name is `live-raid`. `01-create-cluster.sh` saves an ignored repo-local kubeconfig and context under `.runtime/`; every later script loads it automatically when `KUBE_CONTEXT` is unset, including on hosts that inherit a system `KUBECONFIG`. Scripts never change the global kubectl context. Use explicit `IMAGE_REGISTRY`, `IMAGE_TAG`, optional `STORAGE_CLASS`, `IMAGE_PULL_SECRET`, `KUBECONFIG` and `KUBE_CONTEXT` for an existing cluster, then run `./scripts/deploy.sh`. See README for Ingress and LoadBalancer access. Shared S3-compatible checkpoint storage allows Flink workers on different nodes. A remote deployment needs no Docker or k3d once its images are published.

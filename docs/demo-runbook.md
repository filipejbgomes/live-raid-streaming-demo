...
Use `./scripts/reset-demo.sh` for a completely fresh local presentation. It deletes the dedicated `live-raid` k3d cluster, then runs scripts 01–05 to recreate every demo component. Kafka history, checkpoints, verifier evidence and pod restart counts start at zero. It only operates on the named local k3d cluster. `./scripts/cleanup.sh --cluster` deletes that cluster without rebuilding it. Never clear Kafka alone while keeping Flink state.

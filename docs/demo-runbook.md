...
Default local cluster name is `live-raid`. Set `CLUSTER` consistently for local creation and image import. Set `KUBE_CONTEXT=k3d-live-raid` after creation. Deployment and phase scripts use `KUBE_CONTEXT` if provided, otherwise the current context, and never change the global context. Use `IMAGE_REGISTRY`, `IMAGE_TAG`, optional `STORAGE_CLASS` and `IMAGE_PULL_SECRET` for an existing cluster, then `./scripts/deploy.sh`. See README for Ingress and LoadBalancer access. Shared S3-compatible checkpoint storage allows Flink workers on different nodes. A remote deployment needs no Docker or k3d once its images are published.
